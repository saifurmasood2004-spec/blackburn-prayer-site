...
/* Hijri loader (from hijri.json generated by GitHub Action) */
async function loadHijri() {
  try {
    const res = await fetch("./hijri.json", { cache: "no-store" });
    if (!res.ok) return;
    const data = await res.json();
    if (data && data.hijri) hijriText = String(data.hijri);
  } catch {
    // ignore
  }
}
...
function render(todayISO, tomorrowISO, todayRow, tomorrowRow) {
  ...
  // Hijri pill (right side)
  if (els.hijriPill) {
    els.hijriPill.textContent = hijriText || "--";
  }
...
async function init() {
  ...
  // Load Hijri before first render (uses hijri.json created by GitHub Action)
  await loadHijri();
  ...
  setInterval(async () => {
    ...
    if (currentToday !== todayISO) {
      ...
      // Refresh Hijri once per new day
      await loadHijri();
    }
